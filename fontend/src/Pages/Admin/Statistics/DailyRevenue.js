...
const DailyRevenue = () => {
  ...
  const columns =  [
    ...
    {
      title: 'Tiền hàng (giá nhập)',
      dataIndex: 'totalPurchasePrice',
      key: 'totalPurchasePrice',
      render: (totalPurchasePrice) => (
        totalPurchasePrice.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })
      )
    ...
    },
    {
      ...
      key: 'dailyCapital',
      render: (text, record) => {
        const dailySell =  record.totalRevenue;
        const dailyCapital = record.totalPurchasePrice;
        
        
          var profit =  dailySell - dailyCapital ;
   
       
          return   profit.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' });
    ...
    },
    {
      ...
      render: (text, record) => (
        ...
        <Button type="primary" onClick={() => handleShowDetail(record.orderDate)} style={{  textAlign:'center' }}
        >
       Chi tiết
      </Button>
  ...
  const columnsDetail = [
    ...
    },
    {
      ...
      key: 'capital',
      render: (text, record) => {
        const Sell =  record.total;
        const Capital = record.totalPurchasePrice;
        
        
          var profit =  Sell - Capital ;
...
const handleExportDaily = () => {
  ...
  const fileName = 'Doanh thu theo mốc ngày';
...
if(dateorder !== null)
{
  ...
  sheetName = date.toLocaleDateString('en-GB'); // Ví dụ: 'dd-MM-yyyy'
  
  // Loại bỏ các ký tự không hợp lệ trong tên sheet
  ...
  const handleShowDetail =( date) => {

    axiosClient.get(`/Invoices/getinvoicebydaily?date=${date}`)
   .then( res => {
    ...
    const data = res.data.map(item => ({
      ...
      'Tiền vốn': item.totalPurchasePrice,
      'Lợi nhuận': item.total - item.totalPurchasePrice,
    ...
    if(startDate === null && endDate === null ||startDate === null  || endDate === null)
      {
        ...
        axiosClient.get(`/Invoices/daily?startDate=${startDate}&endDate=${endDate}`)
        .then( res => {
          setRevenue(res.data);
          const  data = res.data.map(item =>({
            ...
            'Tiền vốn': item.totalPurchasePrice,
            'Lợi nhuận': item.totalRevenue - item.totalPurchasePrice,
          }));
          setRevenueData(data);
    ...
    return ( <>
      ...
      ( <> <div style={{display:'flex'}}>
            ...
            <div style={{marginLeft:'auto', marginBottom:'5px'}}>
              {RevenueData.length >0 ? handleExportDaily() :null}
            </div>
          </div>
      ...
      { RevenueDetail.length > 0 && dateorder !== null   ?
        ...
        ( <>  <div style={{display:'flex'}}>
                <h3>Danh sách đơn hàng ngày {new Date(dateorder).toLocaleDateString() }</h3>
                <div style={{marginLeft:'auto', marginBottom:'5px'}}>
                {handleExportDay()}
                </div>
              </div>
  ...
  >
  {
    ...
    <h3 className='fs16' style={{color:'red', marginBottom:'5px'}}>Tổng tiền hàng (giá bán) : {formattedSubTotal} </h3>
    ...
    <h3 className='fs16' style={{color:'red'}}>Tổng tiền hàng`(giá bán) `: {formattedSubTotal} </h3>
    <br/>
    <h3 className='fs16' style={{color:'red'}}>Tổng tiền thanh toán : {formattedTotal} </h3>
     </>
  }
